...
typedef struct block_header {
 ...
 __device__ unsigned char* headerChanger(block_header header){
    unsigned char ch[2];
...
__device__ void hex2bin(unsigned char* dest, unsigned char* src)
{
        unsigned char bin;
        int c, pos;
        char buf[3];
...
__device__ void byte_swap(unsigned char* data, int len) {
        int c;
...
__global__ doCalc(block_header *header, int seed) {
    block_header header;
    ...
    header.nonce = seed * blockDim.x + threadIdx.x;
