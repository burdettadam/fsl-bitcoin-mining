...
__global__ void doCalc(unsigned char *dev_merkle_root, unsigned char *dev_prev_block, int seed) {
    int i;   
    block_header header;
    header.version =        2;
...
int main() {
    ...
    unsigned char prev_block[32], dev_prev_block[32], merkle_root[32], dev_merkle_root[32];
    ...
    cudaMemcpy(dev_prev_block, &(prev_block[0]), 32 * sizeof(unsigned char), cudaMemcpyHostToDevice);

    cudaMalloc((void**)&dev_merkle_root, 32*sizeof(unsigned char));
    cudaMemcpy(dev_merkle_root, &(merkle_root[0]), 32 * sizeof(unsigned char), cudaMemcpyHostToDevice);
